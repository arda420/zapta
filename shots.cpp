...
void Shots::OnImpact(IGameEvent* evt) {
	...
	// this is a miss due to wrong angles.
		...
		if (!hit) {
			g_notify.add(tfm::format(XOR("Missed shot due to bad resolve | rmode: %s\n"), g_resolver.resolve_stage[data->m_player->index()]));
		}
		++data->m_missed_shots;
	}
...
void Shots::OnHurt(IGameEvent* evt) {
	...
	if (g_menu.main.misc.hitmarker.get()) {
		...
		switch (g_menu.main.misc.hitsounds.get() % 4) {
		case 0: break;
		case 1: PlaySoundA(reinterpret_cast<char*>(opera), nullptr, SND_ASYNC | SND_MEMORY); break;
		case 2: g_csgo.m_sound->EmitAmbientSound(XOR("buttons/arena_switch_press_02.wav"), 1.f); break;
		case 3: PlaySoundA(reinterpret_cast<const char*>(bameware), nullptr, SND_MEMORY | SND_ASYNC); break;
		}
	}

	// print this shit.
	if (g_menu.main.misc.notifications.get(0)) {
		std::string out = tfm::format(XOR("Fired shot at %s in the %s for %i damage (%i health remaining) | rmode: %s\n"), name, m_groups[group], (int)damage, hp, g_resolver.resolve_stage[data->m_player->index()]);
		g_notify.add(out);
