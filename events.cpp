...
void events::bullet_impact(IGameEvent* evt) {
	...
	if (core.m_processing) {
		...
		Color color = g_menu.main.visuals.impact_beams_color.get();
		//color.a() = (g_menu.main.misc.impact_alpha.get() * 2.55);

		if (g_menu.main.visuals.impact_beams.get())
			g_csgo.m_debug_overlay->AddBoxOverlay(vec3_t(info.x, info.y, info.z), vec3_t(-2, -2, -2), vec3_t(2, 2, 2), ang_t(0, 0, 0), color.r(), color.g(), color.b(), 255, 4.f);
