...
void DrawBeamPaw(vec3_t src, vec3_t end, Color color) {
	...
	*/
	...
	beamInfo.m_flAmplitude = 0.f;   // beam 'jitter'.
	beamInfo.m_flBrightness = 255.f;
	beamInfo.m_flSpeed = 0.5f;  // seems to control how fast the 'scrolling' of beam is... once fully spawned.
	beamInfo.m_nStartFrame = 0;
	beamInfo.m_flFrameRate = 0.f;
	beamInfo.m_nSegments = 2;     // controls how much of the beam is 'split up', usually makes m_flAmplitude and m_flSpeed much more noticeable.
	beamInfo.m_bRenderable = true;  // must be true or you won't see the beam.
	beamInfo.m_nFlags = 0;
	beamInfo.m_flRed = color.r();
	beamInfo.m_flGreen = color.g();
	beamInfo.m_flBlue = color.b();
...
bool c_grenade_prediction::data_t::draw() const {
	...
	if (!g_menu.main.models.grenwarning.get())
		return false;

	if (m_owner != core.m_local && (m_owner->enemy(core.m_local) && !g_menu.main.models.grenwarning.get()))
		return false;

	if (m_owner != core.m_local && (!m_owner->enemy(core.m_local) && !g_menu.main.models.grenwarning.get()))
		return false;

	if (m_owner == core.m_local && !g_menu.main.models.grenwarning.get())
		return false;


	Color color;
		
	if (m_owner == core.m_local)
		color = g_menu.main.models.grenlocal.get();
	else if (m_owner != core.m_local && !m_owner->enemy(core.m_local))
		color = g_menu.main.models.grenfriendly.get();
	else if (m_owner != core.m_local && m_owner->enemy(core.m_local))
		color = g_menu.main.models.grenenemy.get();


	float distance = core.m_local->m_vecOrigin().dist_to(m_origin) / 12.f;
	...
	vec3_t center = core.m_local->WorldSpaceCenter();
	...
	if (trace.m_entity == core.m_local) {
